...
public class MainActivity extends BaseActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LibCollDBInterface _init = new LibCollDBInterface(this);
    ...
    private void addCategory(String name) {
        if (LibCollDBInterface.addCategory(name)) {
            switchContent(currentFragment, HomeFragment.newInstance());
        }
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_home) {
            ...
            new MaterialDialog.Builder(this)
                .title("查看类别列表")
                .items(LibCollDBInterface.getCategories())
                .show();
    ...
    private void remarkBook(String input) {
        if (!LibCollDBInterface.bookExists(input)) {
            new MaterialDialog.Builder(this)
                .content("未在数据库中找到该书籍")
                .positiveText("确定")
                .show();
        ...
        new MaterialDialog.Builder(this)
            ...
            .input(
                "请输入笔记内容", "",
                false,
                (dialog, remark) -> LibCollDBInterface.remarkBook(input, remark.toString())
            )
            .show();
    }
}
